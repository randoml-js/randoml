...
function RandoML(data) {
  ...
  _defineProperty(this, "randomize", function () {
    var unique = false;
    var randomNumber;
    var min = Math.ceil(_this.settings.min);
    var max = Math.floor(_this.settings.max);

    if (typeof _this.methods.onRandomize === 'function') {
      _this.methods.onRandomize();
    }

    do {
      randomNumber = Math.floor(Math.random() * (max - min + 1)) + min;

      var isExcluded = _this.settings.exclude.filter(function (item) {
    ...
    if (typeof _this.methods.onResult === 'function') {
      _this.methods.onResult();
    }
  });

  this.settings = data.settings || {};
  this.methods = data.methods || {}; //this.randomize();
